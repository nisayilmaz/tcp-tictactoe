...
def handle_player(client_socket):
    global board,turn,curr,fin, server_socket
    while True:
        try:
            if started and not fin:
                if turn % 2 == 0:
                    curr = player_sockets[starts]
                else:
                    curr = player_sockets[1 - starts]
                print("Waiting for player", curr, "'s move")
            ...
            if move:
                ...
                if sign == "X":
                    ...
                    sign = 2
                if check_move(x,y):
                    print("Received player", curr, "'s move,", player_signs[player_sockets.index(curr)], "on x:",str(x), "y:", str(y), ",legal move")  
                    board[x][y] = sign
                    result = game()
                    turn = turn + 1 
                    status["status"] = 0
                    ...
                    print("Received player", curr, "'s move,", player_signs[player_sockets.index(curr)], "on x:",str(x), "y:", str(y), ",illegal move")  
                    status["status"] = -1
